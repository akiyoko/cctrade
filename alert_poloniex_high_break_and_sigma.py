...
def get_chart_data(currency_pair, minutes=5, ticks=12*24):
    ...
    df['price_diff_by_sigma'] = (df['close'] - df['rolling_mean']) / df['rolling_std']
    ...
    deviations_by_sigma = ((df.tail(3)['close'] - df.tail(3)['rolling_mean']) / df.tail(3)['rolling_std']).values
    ...
    data = dict(
        ...
        deviations_by_sigma=', '.join(['{:.2f}'.format(v) for v in deviations_by_sigma]),
        sigma_percent='{:.2%}'.format(sigma_percent),
    )
